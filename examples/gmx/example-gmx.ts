    ...
    actions: [
        ...
        action({
            ...
            async handler(
                ...
                try {
                    ...
                    const state = ctx.memory.goal as GmxTradingState;
                    if (state) {
                        state.marketData = {
                    ...
                    if (ordersInfoData) {
                        Object.values(ordersInfoData).forEach(order => {
                            try {
                                ...
                                // Determine order type based on orderType value
                                // 0 = Market Increase, 1 = Limit Increase, 2 = Market Decrease, 3 = Limit Decrease, 4 = Market Swap, 5 = Limit Swap
                                let orderTypeStr = '';
                                let orderDetails = {};
                                
                                // Get market info if available
                                const marketInfo = order.marketAddress ? marketsInfoData[order.marketAddress] : null;
                                const marketName = marketInfo?.name || 'Unknown Market';
                                
                                // ===== IMPROVED ORDER PROCESSING WITH PROPER DECIMAL CONVERSION =====
                                
                                if (order.orderType === 0 || order.orderType === 1) {
                                    // Increase Position Order
                                    const isLimit = order.orderType === 1;
                                    const direction = order.isLong ? 'LONG' : 'SHORT';
                                    
                                    // Get index token symbol from market info
                                    const indexToken = marketInfo?.indexToken?.symbol || 'Unknown';
                                    
                                    // Calculate size in USD using proper decimal conversion
                                    const sizeInUsd = order.sizeDeltaUsd ? 
                                        bigIntToDecimal(order.sizeDeltaUsd, USD_DECIMALS) : 0;
                                    
                                    // Get collateral token info with proper decimal handling
                                    let collateralInfo = {};
                                    if (order.initialCollateralTokenAddress) {
                                        const collateralToken = tokensData[order.initialCollateralTokenAddress];
                                        if (collateralToken) {
                                            const collateralDecimals = collateralToken.decimals || 6;
                                            const collateralAmount = order.initialCollateralDeltaAmount ? 
                                                bigIntToDecimal(order.initialCollateralDeltaAmount, collateralDecimals) : 0;
                                            
                                            // Get token price with proper decimals
                                            const collateralTokenPrice = bigIntToDecimal(
                                    ...
                                    // Process trigger price for limit orders
                                    let triggerPriceValue = null;
                                    if (isLimit && order.triggerPrice) {
                                        triggerPriceValue = bigIntToDecimal(order.triggerPrice, USD_DECIMALS);
                                    } else if (isLimit && order.contractTriggerPrice) {
                                        // Fallback to contractTriggerPrice if available
                                        const triggerPriceDecimals = marketInfo?.indexToken?.decimals || 18;
                                        triggerPriceValue = bigIntToDecimal(order.contractTriggerPrice, triggerPriceDecimals);
                                    }
                                    
                                    const triggerPrice = triggerPriceValue !== null ? 
                                        triggerPriceValue.toFixed(2) : null;
                                    
                                    orderTypeStr = `${isLimit ? 'Limit' : 'Market'} Increase`;
                                    orderDetails = {
                                        action: 'Increase',
                                        market: marketName,
                                        indexToken,
                                    ...
                                    // Decrease Position Order
                                    const isLimit = order.orderType === 3;
                                    const direction = order.isLong ? 'LONG' : 'SHORT';
                                    
                                    // Get index token symbol
                                    const indexToken = marketInfo?.indexToken?.symbol || 'Unknown';
                                    ...
                                    // Handle target collateral token if available
                                    let collateralInfo = {};
                                    if (order.targetCollateralToken) {
                                        collateralInfo = {
                                            receiveToken: order.targetCollateralToken.symbol
                                        };
                                    }
                                    
                                    orderTypeStr = `${isLimit ? 'Limit' : 'Market'} Decrease`;
                                    orderDetails = {
                                        action: 'Decrease',
                                        market: marketName,
                                        indexToken,
                                ...
                                else if (order.orderType === 4 || order.orderType === 5) {
                                    // Swap Order
                                    const isLimit = order.orderType === 5;
                                    
                                    // Get token symbols
                                    let fromTokenSymbol = 'Unknown';
                                    let toTokenSymbol = 'Unknown';
                                    let fromTokenDecimals = 18;
                                    
                                    if (order.initialCollateralTokenAddress && tokensData[order.initialCollateralTokenAddress]) {
                                        const fromToken = tokensData[order.initialCollateralTokenAddress];
                                        fromTokenSymbol = fromToken.symbol;
                                        fromTokenDecimals = fromToken.decimals || 18;
                                    }
                                    
                                    // Get "to" token from swap path if available
                                    if (order.swapPath && order.swapPath.length > 0) {
                                        const lastMarketInPath = order.swapPath[order.swapPath.length - 1];
                                        const swapMarketInfo = marketsInfoData[lastMarketInPath];
                                        if (swapMarketInfo) {
                                            // Guess the output token based on markets structure
                                            toTokenSymbol = (swapMarketInfo.longToken?.symbol || swapMarketInfo.shortToken?.symbol || 'Unknown');
                                        }
                                    } else if (order.swapPathStats && order.swapPathStats.tokenOutAddress) {
                                        // Try to get token out from swapPathStats
                                        const toToken = tokensData[order.swapPathStats.tokenOutAddress];
                                        if (toToken) {
                                            toTokenSymbol = toToken.symbol;
                                        }
                                    }
                                    
                                    // Get amount from initialCollateralDeltaAmount with proper decimal conversion
                                    let amountIn = 0;
                                    let amountInUsd = 0;
                                    
                                    if (order.initialCollateralDeltaAmount && order.initialCollateralTokenAddress) {
                                        const token = tokensData[order.initialCollateralTokenAddress];
                                        if (token) {
                                            const tokenDecimals = token.decimals || 18;
                                            amountIn = bigIntToDecimal(order.initialCollateralDeltaAmount, tokenDecimals);
                                            
                                            // Calculate USD value if token price is available
                                            if (token.prices?.minPrice) {
                                                const tokenPrice = bigIntToDecimal(token.prices.minPrice, USD_DECIMALS);
                                                amountInUsd = amountIn * tokenPrice;
                                            }
                                        }
                                    } else if (order.swapPathStats && order.swapPathStats.amountIn) {
                                        // Try to get amount from swapPathStats
                                        amountIn = bigIntToDecimal(order.swapPathStats.amountIn, fromTokenDecimals);
                                        
                                        // Get USD value if available
                                        if (order.swapPathStats.usdIn) {
                                            amountInUsd = bigIntToDecimal(order.swapPathStats.usdIn, USD_DECIMALS);
                                        }
                                    }
                                    
                                    // Get expected output amount if available
                                    let amountOut = null;
                                    if (order.swapPathStats && order.swapPathStats.amountOut) {
                                        const toToken = tokensData[order.swapPathStats.tokenOutAddress];
                                        if (toToken) {
                                            const toTokenDecimals = toToken.decimals || 18;
                                            amountOut = bigIntToDecimal(order.swapPathStats.amountOut, toTokenDecimals).toFixed(6);
                                        }
                                    }
                                    
                                    // Process trigger ratio for limit swap orders
                                    let triggerRatioInfo = {};
                                    if (isLimit && order.triggerRatio) {
                                        const triggerRatioValue = bigIntToDecimal(order.triggerRatio, 30); // Trigger ratio uses 30 decimals
                                        triggerRatioInfo = {
                                            triggerRatio: triggerRatioValue.toFixed(6)
                                        };
                                    }
                                    
                                    orderTypeStr = `${isLimit ? 'Limit' : 'Market'} Swap`;
                                    orderDetails = {
                                        action: orderTypeStr,
                                        fromToken: fromTokenSymbol,
                                        toToken: toTokenSymbol,
                                        amountIn: amountIn.toFixed(6),
                                        amountInUsd: amountInUsd > 0 ? `$${amountInUsd.toFixed(2)}` : undefined,
                                        amountOut: amountOut,
                                        ...triggerRatioInfo
                                    };
                                }
        ...
        action({
            ...
            async handler(
                ...
                try {                    
                    // Cancel orders
                    const result = await sdk.orders.cancelOrders(call.orderKeys);
                    
                    return {
                        success: true,
                        message: `Successfully submitted cancellation for ${call.orderKeys.length} orders`,
                        result
                    };
                } catch (error) {
                    return {
        ...
        action({
            ...
            async handler(
                ...
                try {                    
                    ...
                    if (call.amount.toLowerCase() === 'max') {
                        ...
                        const tokenInfo = tokensData[call.tokenAddress];
                        
                        if (!tokenInfo) {
                            throw new Error(`Token not found: ${call.tokenAddress}`);
                        }
                        
                        // Set approval to equivalent of 100,000 tokens (which should be plenty for trading)
                        const decimalMultiplier = BigInt(10) ** BigInt(tokenInfo.decimals || 18);
                        approvalAmount = BigInt(100000) * decimalMultiplier;
                    } else {
                        // Get token info to calculate decimals
                        const tokensDataResult = await sdk.tokens.getTokensData();
                        const tokensData = tokensDataResult.tokensData || {};
                        ...
                        const decimalMultiplier = BigInt(10) ** BigInt(tokenInfo.decimals || 18);
                        const baseAmount = BigInt(Math.floor(parseFloat(call.amount) * Number(decimalMultiplier)));
                        approvalAmount = baseAmount * BigInt(10); // 10x the requested amount
                    }
                    
                    console.log(`Approving ${call.tokenAddress} for ${approvalAmount} to spender ${exchangeRouterAddress}`);
                    
                    // Create and send approval transaction
                    const result = await sdk.callContract(
                        call.tokenAddress as `0x${string}`,
                        tokenAbi,
                        "approve",
                    ...
                    return {
                        success: true,
                        message: `Successfully submitted approval for ${call.amount} of token ${call.tokenAddress}`,
        ...
        action({
            ...
            async handler(
                ...
                try {
                    ...
                    // Get market info
                    const marketInfo = marketsInfoData[call.marketAddress];
                    if (!marketInfo) {
                        throw new Error(`Market not found: ${call.marketAddress}`);
                    }
                    
                    // Get collateral token
                    const collateralToken = tokensData[call.collateralTokenAddress];
                    if (!collateralToken) {
                        throw new Error(`Collateral token not found: ${call.collateralTokenAddress}`);
                    }
                    
                    // Get index token
                    const indexToken = tokensData[marketInfo.indexTokenAddress];
                    if (!indexToken) {
                        throw new Error(`Index token not found for market: ${call.marketAddress}`);
                    }
                    
                    // Calculate a reasonable position sizes with proper precision
                    // GMX uses 30 decimals for USD amounts
                    const sizeDeltaUsd = BigInt(Math.floor(call.sizeUsd * (10 ** USD_DECIMALS)));
                    const collateralUsd = sizeDeltaUsd / BigInt(call.leverage);
                    
                    // Get prices for tokens - these come from the SDK data
                    const indexTokenPrice = indexToken.prices?.maxPrice || BigInt(0);
                    const collateralTokenPrice = collateralToken.prices?.minPrice || BigInt(0);
                    ...
                    const isLong = call.isLong;
                    
                    // For long positions, we're willing to pay more, for shorts we want to receive more
                    const acceptablePriceMultiplier = isLong 
                        ? BigInt(10000) + acceptablePriceDeltaBps 
                        : BigInt(10000) - acceptablePriceDeltaBps;
                    
                    const acceptablePrice = (indexTokenPrice * acceptablePriceMultiplier) / BigInt(10000);
                    ...
                    let triggerPrice = BigInt(0);
                    if (call.triggerPrice) {
                        triggerPrice = BigInt(parseFloat(call.triggerPrice) * (10 ** USD_DECIMALS));
                    }
                    ...
                    const increaseAmounts = {
                        ...
                        estimatedLeverage: BigInt(Math.floor(call.leverage * 10000)),
                        ...
                        estimatedPnl: BigInt(0),
                        estimatedPnlPercentage: BigInt(0),
                    ...
                    console.log("Submitting increase order with amounts:", JSON.stringify({
                        marketAddress: call.marketAddress,
                        isLong: call.isLong,
                        isLimit: call.isLimit,
                        leverage: call.leverage,
                        sizeUsd: call.sizeUsd,
        ...
        action({
            ...
            async handler(
                ...
                try {
                    ...
                    const acceptablePriceMultiplier = call.isLong 
                        ? BigInt(10000) - acceptablePriceDeltaBps 
                        : BigInt(10000) + acceptablePriceDeltaBps;
                    ...
                    const decreaseAmounts: any = {
                        ...
                        collateralDeltaAmount: BigInt(0), // Will be calculated by GMX

                        indexPrice: indexTokenPrice,
                        collateralPrice: collateralToken.prices?.minPrice || BigInt(0),
                        triggerPrice: call.isLimit ? triggerPrice : undefined,
                        acceptablePrice,
                        ...
                        realizedPnlPercentage: BigInt(0),
                        ...
                        positionPriceImpactDeltaUsd: BigInt(0),
                        priceImpactDiffUsd: BigInt(0),
                        payedRemainingCollateralAmount: BigInt(0),
                        
                        payedOutputUsd: BigInt(0),
                        payedRemainingCollateralUsd: BigInt(0),
                        
                        receiveTokenAmount: BigInt(0),
                        receiveUsd: BigInt(0),
                        
                        // Set the order type based on isLimit
                        triggerOrderType: call.isLimit ? 
        ...
        action({
            ...
            async handler(
                ...
                try {
                    ...
                    // Get tokens
                    const fromToken = tokensData[call.fromTokenAddress];
                    if (!fromToken) {
                        throw new Error(`From token not found: ${call.fromTokenAddress}`);
                    }
                    
                    const toToken = tokensData[call.toTokenAddress];
                    if (!toToken) {
                        throw new Error(`To token not found: ${call.toTokenAddress}`);
                    }
                    ...
                    for (const [address, market] of Object.entries(marketsInfoData)) {
                        ...
                        const hasFromToken = 
                            market.longTokenAddress === call.fromTokenAddress || 
                            market.shortTokenAddress === call.fromTokenAddress;
                        
                        const hasToToken = 
                            market.longTokenAddress === call.toTokenAddress || 
                            market.shortTokenAddress === call.toTokenAddress;
                    ...
                    if (!marketKey) {
                        ...
                        for (const [address, market] of Object.entries(marketsInfoData)) {
                            ...
                            if (!fromTokenMarket && 
                                (market.longTokenAddress === call.fromTokenAddress || market.shortTokenAddress === call.fromTokenAddress) &&
                            ...
                            if (!toTokenMarket && 
                                (market.longTokenAddress === call.toTokenAddress || market.shortTokenAddress === call.toTokenAddress) &&
                                (market.longTokenAddress === usdcAddress || market.shortTokenAddress === usdcAddress)) {
                    ...
                    const amountIn = BigInt(Math.floor(call.amountIn * Number(`1e${fromToken.decimals || 18}`)));
                    
                    // Get token prices
                    const fromTokenPrice = fromToken.prices?.minPrice || BigInt(0);
                    const toTokenPrice = toToken.prices?.maxPrice || BigInt(0);
                    
                    // Calculate USD values
                    const usdIn = amountIn * fromTokenPrice / BigInt(10 ** (fromToken.decimals || 18));
                    
                    // Calculate estimated output amount with 0.3% fee
                    const feeAmount = usdIn * BigInt(30) / BigInt(10000); // 0.3% fee
                    ...
                    const minOutputAmount = (estimatedAmountOut * (BigInt(10000) - slippageBps)) / BigInt(10000);
                    ...
                    const swapPathStats = {
                        ...
                        swapSteps: [{
                            ...
                            tokenInAddress: call.fromTokenAddress,
                            tokenOutAddress: call.toTokenAddress,
                            isWrap: false,
                            isUnwrap: false,
                            ...
                            amountIn,
                            amountInAfterFees: amountIn - (amountIn * BigInt(30) / BigInt(10000)),
                            usdIn,
                            amountOut: estimatedAmountOut,
                            usdOut
                        }],
                        totalSwapPriceImpactDeltaUsd: BigInt(0),
...
const agent = createDreams({
    ...
    model: anthropic("claude-3-5-sonnet-latest"), //groq("qwen-2.5-32b"), //
...
agent.start({
    id: "gmx-trading",
    initialGoal: "Help the user trade on GMX following a systematic approach with proper risk management. Always use USD as collateral.",
    initialTasks: [
        "Monitor market conditions",
