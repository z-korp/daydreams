        ...
        const handler: ActionHandler<typeof zodSchema, any, any, Agent> = async (
            ...
            try {
                if (func.state_mutability === "view") {
                    ...
                    if (gameState.adventurerState || gameState.marketItems !== undefined) {
                        ...
                        if (lootSurvivorCtxDef && args.adventurer_id) {
                            ...
                            if (gameState.adventurerState) {
                                ...
                                if (gameState.adventurerState.adventurer.statUpgradesAvailable === 0) {
                        ...
                        if (adv.statUpgradesAvailable > 0 && gameState.marketItems) {
                            result.marketInfo = {
                                ...
                                items: gameState.marketItems.map(id => {
                                    ...
                                    const discount = Math.min(adv.stats.charisma * ITEM_CHARISMA_DISCOUNT, basePrice - ITEM_MINIMUM_PRICE);
                                ...
                                potionPrice: Math.max(POTION_BASE_PRICE - (adv.stats.charisma * POTION_CHARISMA_DISCOUNT), 0)
                            };
                        }
                    }
    ...
    handler: async (
        ...
        try {
            ...
            if (result.success && result.result) {
                ...
                const maxHealth = 50 + (adventurerPart.stats.vitality * 15) + ((level - 1) * 15);
                const healthPercent = Math.round((adventurerPart.health / maxHealth) * 100);
                ...
                const formatEquipmentItem = (item: { id: number; xp: number }) => {
                    ...
                    const lootManager = new LootManager(item.id, item.xp, BigInt(updatedAdventurerState.adventurerEntropy || '0'));
                    const itemType = lootManager.getItemType();
                    const itemLevel = item.xp > 0 ? Math.floor(Math.sqrt(item.xp)) : 0;
                    return `${itemName} [${itemType}]${itemLevel > 0 ? ` (Lvl ${itemLevel})` : ''}`;
                };
                
                const summary = `Adventurer #${adventurerId} Status:
` +
...
` +
                    `${adventurerPart.beastHealth > 0 ? `⚔️ IN COMBAT - Beast Health: ${adventurerPart.beastHealth}\n` : ''}` +
                    `${adventurerPart.statUpgradesAvailable > 0 ? `📈 ${adventurerPart.statUpgradesAvailable} stat upgrades available!\n` : ''}` +
                    ...
                    `\nEquipment:\n` +
                    `- Weapon: ${formatEquipmentItem(adventurerPart.equipment.weapon)}\n` +
