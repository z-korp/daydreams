...
## 1. Environment setup and blockchain client
...
The blockchain game agent demonstrates on-chain game integration:

1. **State synchronization**: Fetches game state from blockchain contracts
2. **Transaction management**: Submits game actions as blockchain transactions
3. **Game logic validation**: Checks game rules before performing actions
4. **Progress tracking**: Manages experience, levels, and equipment progression

**Example gameplay:**

```
User: "Check my game status"
Agent: *calls getGameState action*
Agent: "You're level 1 with 100/100 health. You have a Basic Sword and Basic Armor."

User: "Go explore"
Agent: *calls explore action*
Agent: "Encountered Goblin Warrior! Prepare for battle."

User: "Attack the enemy"
Agent: *calls attack action*
Agent: "Dealt 15 damage, took 8 damage. Enemy health: 35/50"

User: "I leveled up, allocate my stats"
Agent: *calls allocateStats action*
Agent: "Allocated 3 stat points: +2 STR, +1 VIT. Max health increased to 110."
```

The agent manages blockchain transactions, validates game state, and makes
strategic decisions based on current player status and available actions.
