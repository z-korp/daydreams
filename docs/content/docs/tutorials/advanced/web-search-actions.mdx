...
The web search agent demonstrates custom action development:

1. **Service injection**: The `install` hook registers external services with
   the container
2. **Error handling**: Actions handle network failures and API errors gracefully
3. **Input validation**: Zod schemas validate and type action parameters
4. **Response formatting**: Actions return structured data the LLM can
   understand
...
Agent: *calls searchWeb action*
Agent: "Found 5 results about TypeScript. It's a strongly typed programming language..."

User: "Get more details from the official TypeScript website"
Agent: *calls fetchUrl action*
Agent: "Fetched content from typescriptlang.org. Here are the key details..."

User: "Find the Wikipedia article about JavaScript"
Agent: *calls searchWikipedia action*
Agent: "Found Wikipedia article: JavaScript is a programming language..."
```

The actions enable the agent to access current web information, fetch specific
pages, and search specialized sources like Wikipedia.
