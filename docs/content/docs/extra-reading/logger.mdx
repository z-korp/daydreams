...
-   _"How can I change the logging level (e.g., see DEBUG messages)?"_
    You can set the `logLevel` option when calling `createDreams`. For example: `createDreams({ ..., logLevel: LogLevel.DEBUG })`. The levels are `ERROR`, `WARN`, `INFO`, `DEBUG`, `TRACE` (most verbose).
-   _"Can I send logs somewhere other than the console?"_
    Yes, the logger is designed with "transports". While the default is `ConsoleTransport`, you could potentially implement custom transports (though this is an advanced topic not covered here) and provide them via the `logger` or `transports` option in `createDreams`.
-   _"Why provide a `context` string (like `'processData:handler'`)?"_
    It helps identify _where_ in the code the log message originated, which is very useful for debugging complex agents. 