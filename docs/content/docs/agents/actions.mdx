...
action({
  ...
  handler(call, ctx, agent) {
    const { query, limit = 10 } = call.data;
...
action({
  ...
  handler(call, ctx, agent) {
    ...
    return {
      task: newTask,
      message: `Added task "${call.data.title}" with ${call.data.priority} priority`,
    };
  },
});
```
...
action({
  ...
  async handler(call, ctx, agent) {
    try {
      ...
      return {
        success: true,
        message: `Email sent to ${call.data.to}`,
      };
    } catch (error) {
      return {
        success: false,
...
action({
  ...
  async handler(call, ctx, agent) {
    const data = await weatherAPI.fetch(call.data.location);

    // Store in context for other actions to use
    ctx.agentMemory.weatherData = data;
...
action({
  ...
  handler(call, ctx, agent) {
    ...
    const taskIndex = tasks.findIndex((t) => t.id === call.data.taskId);

    if (taskIndex === -1) {
      return {
        success: false,
        message: `Task with ID ${call.data.taskId} not found`,
      };
    }
...
action({
  ...
  handler(call, ctx, agent) {
    const { title, content, tags = [] } = call.data;
...
action({
  ...
  handler(call, ctx, agent) {
    ...
    const { query, searchIn } = call.data;
...
action({
  ...
  handler(call, ctx, agent) {
    const { documents = [] } = ctx.agentMemory;
    const { documentId, updates } = call.data;
