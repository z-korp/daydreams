...
agent.run();`;
        ...
        const tsconfigCall = writeFileCalls.find(
            (call) =>
                typeof call[0] === "string" && call[0].endsWith("tsconfig.json")
        );
        const indexCall = writeFileCalls.find(
            (call) =>
                typeof call[0] === "string" && call[0].endsWith("index.ts")
        );
        const envCall = writeFileCalls.find(
            (call) =>
                typeof call[0] === "string" && call[0].endsWith(".env.example")
        );
        const readmeCall = writeFileCalls.find(
            (call) =>
                typeof call[0] === "string" && call[0].endsWith("README.md")
        );
