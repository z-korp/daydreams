...
export class ChainOfThoughtProcessor extends BaseProcessor {
  ...
  public async process(
    ...
    this.logger.debug("ChainOfThoughtProcessor.process", "Processing content", {
      content,
    });
    ...
    try {
      ...
      while (true) {
        const readyGoals = this.chainOfThought.goalManager.getReadyGoals();
        ...
        if (readyGoals.length === 0 && activeGoals.length === 0) {
          ...
          pendingGoals.forEach((goal) => {
            const blockingGoals =
              this.chainOfThought.goalManager.getBlockingGoals(goal.id);
            this.logger.warn(
              "ChainOfThoughtProcessor.process",
      ...
      return {
        ...
        enrichedContext: {
          ...
          availableOutputs: ioContext?.availableOutputs?.map((h) => h.name),
          ...context,
        },
