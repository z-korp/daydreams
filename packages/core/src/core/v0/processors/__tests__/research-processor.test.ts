...
const mockCharacter: Character = {
  ...
  traits: [
    {
      name: "analytical",
      description: "Focuses on systematic analysis of data and information",
      strength: 0.9,
    ...
    },
    {
      ...
      strength: 0.8,
      examples: ["Catches minor discrepancies", "Thorough in examination"],
    },
    {
      name: "objective",
    ...
    complete: vi.fn().mockImplementation((prompt) => {
      return Promise.resolve({
        extractedData: {
          ...
          quotes: ["The resilience of consumer spending has been remarkable"],
          ...
          keyInsights: [
            {
              ...
              confidence: 0.9,
              supportingEvidence: ["GDP growth reached 3.2% in Q1"],
            },
          ],
      ...
      try {
        ...
        if (result.enrichedContext) {
          ...
          if (result.enrichedContext.keyInsights) {
            expect(Array.isArray(result.enrichedContext.keyInsights)).toBe(
              true
            );
            if (result.enrichedContext.keyInsights.length > 0) {
              const firstInsight = result.enrichedContext.keyInsights[0];
        ...
        if (result.extractedData) {
          // Check arrays exist
          expect(Array.isArray(result.extractedData.numericalData)).toBe(true);
