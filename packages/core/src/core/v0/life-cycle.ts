...
export function makeFlowLifecycle(
  ...
  return {
    ...
    async onContentProcessed(
      ...
      await conversationManager.markContentAsProcessed(contentId, threadId);
    },
    ...
    async onMemoryAdded(
      ...
      await orchestratorDb.addChatMessage(chatId, HandlerRole.INPUT, source, {
        content,
        chatId,
      });
    },
    ...
    async onMemoriesRequested(
      ...
      const memories = await conversationManager.getMemoriesFromConversation(
        chatId,
        limit
      );
...
export interface FlowLifecycle {
   ...
   */
  onCheckContentProcessed(contentId: string, chatId: string): Promise<boolean>;
}
