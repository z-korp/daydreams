...
export class ConversationManager {
  ...
  public async getConversation(
    ...
    try {
      const collection =
        await this.vectorDb.getCollectionForConversation(conversationId);
    ...
    try {
      ...
      await collection.modify({
        metadata: {
          ...
          lastActive: conversation.getMetadata().lastActive.toISOString(),
          name: metadata?.name,
  ...
  public async addMemory(
    ...
    await this.vectorDb.storeInConversation(memory.content, conversation.id, {
      memoryId: memory.id,
  ...
  public async ensureConversation(
    ...
    let conversation = await this.getConversationByPlatformId(name, platform);
    if (!conversation) {
  ...
  public async markContentAsProcessed(
    ...
    if (!this.vectorDb) {
      throw new Error("VectorDB required for marking content as processed");
    }
