...
export class TelegramClient {
  ...
  private async sendStartupMessage(user: Api.User): Promise<void> {
    ...
    try {
      ...
      this.logger.error("TelegramClient", "Failed to send startup message", {
        error: msgError,
      });
    }
  }
          ...
          if (env.TELEGRAM_STARTUP_CHAT_ID) {
            ...
            const chatChunks = chats
              ...
              .reduce((acc: string[], chat, index) => {
                ...
                if (
                  !acc.length ||
                  acc[acc.length - 1].length + chatInfo.length > 4000
                ) {
                  acc.push(chatInfo);
                } else {
                  acc[acc.length - 1] += "\n" + chatInfo;
  ...
  private async sendMessage(
    ...
    try {
      ...
      this.logger.error("TelegramClient.sendMessage", "Error sending message", {
        error,
      });
      throw error;
    }
  ...
  public async getChats(): Promise<ChatInfo[]> {
    try {
      ...
      this.logger.error("TelegramClient.getChats", "Error fetching chats", {
        error,
      });
      throw error;
    }
